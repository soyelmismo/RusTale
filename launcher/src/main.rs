...
use crate::ui::{background_blur, control_section, lsd_shader, profile_card}; // Importar structs
...
impl RusTale {
    ...
    fn new(quickplay: bool) -> (Self, Task<Message>) {
        ...
        let bg_task = if initial_bg_bytes.is_some() {
            ...
            Task::perform(
                async move {
                    util::image_cache::process_background_async(&client_for_bg, bg_url).await?;
    ...
    fn subscription(&self) -> Subscription<Message> {
        ...
        let memory_stats_sub = iced::time::every(std::time::Duration::from_secs(5))
            .map(|_| Message::MemoryStatsUpdate);
                ...
                if let Event::Keyboard(iced::keyboard::Event::KeyPressed { key, .. }) = event {
                    match key {
                        iced::keyboard::Key::Named(iced::keyboard::key::Named::ArrowRight) => {
                            Some(Message::NextShaderManual)
                        }

                        iced::keyboard::Key::Character(s) if s.as_str() == "s" => {
                            Some(Message::NextShaderManual)
                        }
                        ...
                        iced::keyboard::Key::Character(a) if a.as_str() == "a" => {
                            Some(Message::NextShaderManual)
                        }
        ...
        match message {
            ...
            Message::ProgressUpdate(payload) => {
                ...
                let progress_changed =
                    (payload.global_progress * 100.0 - self.last_download_progress).abs() > 0.1;
                if progress_changed {
                ...
                if payload.message_key.contains("step") || payload.message_key.contains("phase") {
                    ...
                    if let Some(total) = self.total_steps {
                        ...
                        let estimated_step =
                            ((progress_percent / 100.0) * total as f32).ceil() as usize;
            ...
            Message::News(msg) => {
                ...
                self.views
                    .news
                    .update(msg, self.services.api_client.clone())
            }
