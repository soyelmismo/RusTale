...
impl EstrogenProvider {
    ...
    async fn get_available_versions_search(
        &self,
        architecture: &str,
        operating_system: &str,
        channel: &str,
        ...
        let latest = find_latest_version_generic(0, |version| async move {
            self.check_version_exists(0, version, architecture, operating_system, channel)
                .await
        })
        .await;
        ...
        for v in 0..=latest {
            if self
                .check_version_exists(0, v, architecture, operating_system, channel)
                .await
            {
                versions.push(v);
            }
        ...
        if versions.is_empty() {
            anyhow::bail!(
                "No versions found for channel {} on {}-{}",
                channel,
                operating_system,
                architecture
            );
...
impl PatchProvider for EstrogenProvider {
    ...
    async fn get_latest_version(&self, channel: &str, os: &str, arch: &str) -> Result<i32> {
        let version = find_latest_version_generic(0, |version| async move {
            self.check_version_exists(0, version, arch, os, channel)
                .await
        })
        .await;
        Ok(version)
    ...
    async fn get_patch_url(
        ...
        if !self
            .check_version_exists(from_version, to_version, arch, os, channel)
            .await
        {
            anyhow::bail!(
