...
impl UiOrchestrator {
    ...
    pub fn handle_core_event(
        ...
        match event {
            ...
            FromCore::BootstrapFailed(err) => {
                // Set status_text so the error is readable in the control section and reset to
                // Ready so the user can still interact (e.g. retry via the Play button).
            ...
            FromCore::NewsLoaded(result) => {
                // Delegate through NewsSection::update(NewsLoaded) which handles loading state,
                // image loading tasks, and error state reporting correctly.
            ...
            FromCore::ModOperationFinished(result) => {
                match result {
                    Ok(_) => {
                        // Trigger a background refresh task so the mods list reflects
                        // the toggle/uninstall when the mods view is open.
            ...
            FromCore::LauncherUpdateCheckResult(result) => {
                // Update the settings modal's update_btn_status so the
            ...
            FromCore::ModInstallCompleted(result) => {
                match result {
                    Ok(mod_id) => {
                        ...
                        // Dispatch a background refresh task when the mods view is open to reload local mods.
                        if self.views.mods.is_open {
                            return Some(Task::done(Message::Mods(
                                crate::ui::mods_modal::ModsMessage::RefreshLocalBackground,
                            )));
    ...
    fn handle_ui_event_with_core(
        ...
        match message {
            ...
            // === CAMBIO DE IDIOMA ===
            // Route language changes to handle_visual_events to reload localization immediately.
    ...
    fn handle_game_events_with_core(
        ...
        match message {
            Message::StartGame => {
                // When the game is already running, route to ToCore::StopGame instead of LaunchGame.
            ...
            Message::GameLaunched(result) => {
                match result {
                    ...
                    Err(e) => {
                        // Setting Ready allows the user to retry. The coordinator always sends
                        // StatusChanged(Ready) on LaunchFailed via the internal channel, so this
                        // branch mirrors that behavior for any legacy callers of Message::GameLaunched.
                        self.error = Some(e.clone());
                        self.displayed_status = crate::game::LauncherStatus::Ready;
                    }
                }
    ...
    fn handle_version_events_with_core(
        ...
        match message {
            ...
            Message::RequestDeleteVersion(version) => {
                ...
                // After deleting a version, dispatch CheckStatus which sends RequestInitialStatus
                // to scan local versions and refresh InstalledVersionsLoaded automatically.
                Some(Task::done(Message::CheckStatus))
            }
    ...
    fn handle_migration_events_with_core(
        ...
        match message {
            ...
            Message::CancelAction => {
                // Forward to the engine to cancel all managed tasks.
