...
use crate::core::logic::{mods_loader, profiles, launcher};
use anyhow;
use std::sync::Arc;
use tokio::sync::{mpsc, oneshot};
use rustale_shared::java::tracking::{get_tracked_pids, is_tracked_process, cleanup_old_pids, clear_all_tracked_pids, untrack_process};
...
async fn kill_java_processes() -> anyhow::Result<()> {
    ...
    println!("[Core] Found {} tracked PIDs to check: {:?}", tracked_pids.len(), tracked_pids);
    ...
    for pid in tracked_pids {
        ...
        if is_tracked_process(pid) {
            ...
            println!("[Core] PID {} no longer valid or not a Java process, skipping", pid);
        }
    }
...
pub enum CoordinatorEvent {
    GameProcessReady(tokio::process::Child, Option<oneshot::Sender<()>>, Option<crate::game::aurora::FileCleanupGuard>),
    LaunchFailed(CoreError),
}

pub async fn run(mut rx: mpsc::Receiver<ToCore>, tx: mpsc::Sender<FromCore>, initial_profiles: Option<ProfilesConfig>) {
...
async fn handle_internal_event(event: CoordinatorEvent, state: &mut LogicState, tx: &mpsc::Sender<FromCore>) {
    match event {
        CoordinatorEvent::GameProcessReady(mut child, auth_stop, sec_guard) => {
            ...
            if state.stop_requested {
                println!("[Core] Discarding stale GameProcessReady: stop was requested mid-launch, killing child.");
            ...
            let _ = tx.send(FromCore::StatusChanged(LauncherStatus::Playing)).await;
        },
        ...
        CoordinatorEvent::LaunchFailed(error) => {
            ...
            let _ = tx.send(FromCore::Error { message: error.to_string(), fatal: false }).await;
            let _ = tx.send(FromCore::StatusChanged(LauncherStatus::Ready)).await;
        }
    }
}
...
async fn handle_ui_message(
    ...
    match msg {
        ToCore::BootstrapSystem => {
            ...
            // 4. Send State to UI
            let _ = tx.send(FromCore::BootstrapCompleted { 
                settings, 
                profiles: profiles
            }).await;
        }
        ...
        ToCore::RequestInitialStatus(settings) => {
            ...
            if state.version_service.is_none() {
                ...
                state.version_service = Some(crate::core::services::version_service::VersionService::new(
                    system::get_app_dir()
                ));
            }
            ...
            state.spawn_managed(TaskType::GenericIO, move |_| async move {
                ...
                let svc = crate::core::services::version_service::VersionService::new(paths.root.clone());
                
                // Fetch Remote
                println!("[Core] Fetching remote versions");
                let latest_remote = match svc.refresh_versions(&settings_clone.channel, &tx_clone).await {
                    Ok(v) => {
        ...
        ToCore::LaunchGame => {
            ...
            crate::core::services::launcher_service::LauncherService::launch(state, tx.clone(), internal_tx.clone(), &localization).await;
        }
        ...
        ToCore::MigrateData { from, to } => {
            ...
            state.spawn_managed(TaskType::GenericIO, move |cancel_token| async move {
                ...
                let _ = tx_clone
                    .send(FromCore::MigrationFinished(
                        result
                            .map(|_| dest)
                            .map_err(|e: anyhow::Error| CoreError::GenericError(e.to_string())),
                    ))
                    .await;
            });
        }
        ToCore::AbortOperation => {
            ...
            if !active_tasks.is_empty() {
                println!("[Core] Aborting {} active tasks: {:?}", active_tasks.len(), active_tasks);
            }
            state.cancel_all();
            let _ = tx.send(FromCore::StatusChanged(LauncherStatus::Ready)).await;
        }
        ...
        ToCore::StopGame => {
            ...
            if let Some(mut child) = state.game_process.take() {
                ...
                println!("[Core] StopGame: no running process (launch may be in-flight; stop_requested set).");
            }
           ...
           // The original `state.spawn_managed` call is kept.
            ...
            state.spawn_managed(TaskType::AppUpdate, move |_cancel_token| async move {
                ...
                match res {
                    ...
                    Err(e) => {
                        let _ = tx_clone
                            .send(FromCore::LauncherUpdateCheckResult(Err(CoreError::NetworkError(e.to_string()))))
                            .await;
                    }
                }
            });
        ...
        ToCore::SearchMods {
            ...
            state.spawn_managed(TaskType::ModSearch, move |_cancel_token| async move {
                ...
                let _ = tx
                    .send(FromCore::ModsSearchLoaded(res.map_err(|e| CoreError::GenericError(e.to_string()))))
                    .await;
            });
        }
        ToCore::LoadLocalMods { channel, version } => {
            ...
            state.spawn_managed(task_id, move |cancel_token| async move {
                ...
                // Perform load...
                let res = mods_service.load_local_mods(base_dir.clone(), channel.clone(), version.clone()).await;
                ...
                if res.is_ok() {
                     ...
                     if !cancel_token.load(std::sync::atomic::Ordering::Relaxed) {
                         if let Err(e) = crate::game::zip_mods::verify_patch_integrity(&paths, &channel, &version) {
                             eprintln!("[Core] Warning: Patch integrity verification failed: {}", e);
                         }
                     }
                }
                
                let _ = tx.send(FromCore::LocalModsLoaded(
        ...
        ToCore::InstallMod(req) => {
            ...
            if state.is_task_running(&TaskType::ModInstallation(req.mod_id.clone())) {
                println!("[Core] Mod installation already in progress for: {}", req.mod_id);
                let _ = tx.send(FromCore::Error {
                    message: format!("Mod installation already in progress for: {}", req.mod_id),
                    fatal: false,
                }).await;
                return;
            }
            ...
            state.spawn_managed(TaskType::ModInstallation(req.mod_id.clone()), move |cancel_token| async move {
                let settings = system::load_settings().await;
                let reporter = Arc::new(ModReporter { tx: tx.clone() });
                let res = mods_service.install_mod(
                    req,
                ...
                let _ = tx
                    .send(FromCore::ModOperationFinished(
                        res.map(|_mod_id| ()).map_err(|e| CoreError::GenericError(e.to_string())),
                    ))
                    .await;
            });
        }
        ToCore::UninstallMod(mod_id) => {
            ...
            state.spawn_managed(TaskType::ModInstallation(mod_id.clone()), move |cancel_token| async move {
                let settings = system::load_settings().await;
                // Use the optimized ModsService for centralized uninstallation
                let mods_service = mods_loader::ModsService::new(client.clone());
                let reporter = Arc::new(ModReporter { tx: tx.clone() });
                let res = mods_service.uninstall_mod(
                    mod_id.clone(),
                    settings.channel.clone(),
                    settings.game_version,
                    base_dir,
        ...
        ToCore::ToggleMod(name, enabled) => {
            ...
            state.spawn_managed(TaskType::ModInstallation(name.clone()), move |cancel_token| async move {
                // Check cancellation before expensive operation
                if cancel_token.load(std::sync::atomic::Ordering::Relaxed) { return; }
                ...
                let mods_service = mods_loader::ModsService::new(client.clone());
                let res = mods_service.toggle_mod(
                    name,
                    enabled,
                    base_dir,
                    settings.channel,
        ...
        ToCore::ToggleZipPatch(id, enabled) => {
            ...
            state.spawn_managed(TaskType::ModInstallation(format!("patch_{}", id)), move |cancel_token| async move {
                // Check cancellation before expensive operation
                if cancel_token.load(std::sync::atomic::Ordering::Relaxed) { return; }
                ...
                let mods_service = mods_loader::ModsService::new(client.clone());
                let res = mods_service.toggle_patch(
                    id,
                    enabled,
                    base_dir,
                    settings.channel,
            ...
            if let Some(manager) = &mut state.profile_manager {
                ...
                match validation_result {
                    Ok(_) => {
                        ...
                        let _ = tx_clone.send(FromCore::ProgressUpdate {
                        ...
                        match manager.import_profiles(&path).await {
                            Ok(count) => {
                                ...
                                if let Err(e) = manager.save_profiles().await {
                                    ...
                                    let _ = tx_clone.send(FromCore::Error {
                                        message: format!("Successfully imported {} profiles", count),
                                        fatal: false,
                                    }).await;
                                    let _ = tx_clone.send(FromCore::SettingsSaved).await;
                                }
                ...
                match manager.import_profiles_from_vec(&profiles) {
                    Ok(count) => {
                        ...
                        if let Err(e) = manager.save_profiles().await {
                            ...
                            let _ = tx_clone.send(FromCore::Error {
                                message: format!("Successfully imported {} profiles from memory", count),
                                fatal: false,
                            }).await;
                            let _ = tx_clone.send(FromCore::SettingsSaved).await;
                        }
        ...
        ToCore::CheckForUpdates => {
            ...
            if state.is_task_running(&TaskType::AppUpdate) { 
                println!("[Core] App update check already running, skipping generic update check request");
                return; 
            } 
            ...
            state.spawn_managed(TaskType::AppUpdate, move |cancel_token| async move {
                ...
                match mods_service.check_updates(manifest, patches, &version_str).await {
                    ...
                    Err(e) => {
                         let _ = tx
                            .send(FromCore::UpdatesLoaded(Err(CoreError::GenericError(e.to_string()))))
                            .await;
                    }
                }
            });
        ...
        ToCore::LoadVersions(mod_id) => {
            ...
            if state.is_task_running(&TaskType::ModInstallation(format!("versions_{}", mod_id))) {
                println!("[Core] Version loading already in progress for mod: {}", mod_id);
                return;
            }
            
            let tx = tx.clone();
            let client = state.http_client.clone();
            state.spawn_managed(TaskType::ModInstallation(format!("versions_{}", mod_id)), move |cancel_token| async move {
                // Check cancellation before expensive operation
                if cancel_token.load(std::sync::atomic::Ordering::Relaxed) { return; }
                ...
                match mods_service.get_versions(&mod_id).await {
                    ...
                    Err(e) => {
                        ...
                        if cancel_token.load(std::sync::atomic::Ordering::Relaxed) { return; }
                        let _ = tx
                            .send(FromCore::VersionsLoaded(Err(CoreError::GenericError(e.to_string()))))
                            .await;
                    }
                }
            });
        ...
        ToCore::GetCacheStats => {
            let tx = tx.clone();
            state.spawn_managed(TaskType::GenericIO, move |_| async move {
                let cache_stats = crate::game::patch_api::get_shared_cache().get_cache_stats().await;
                let _ = tx.send(FromCore::CacheStatsLoaded(
        ...
        ToCore::FetchNews => {
            ...
            state.spawn_managed(TaskType::GenericIO, move |_| async move {
                match crate::news::fetch_news().await {
                    ...
                    Err(e) => {
                        let _ = tx.send(FromCore::NewsLoaded(Err(CoreError::NetworkError(e.to_string())))).await;
                    }
                }
            });
        }
