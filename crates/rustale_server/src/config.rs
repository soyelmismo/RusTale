...
pub async fn load_or_create(args: &rustale_engine::cli::CliArgs) -> ServerConfig {
    ...
    if config.server_args.contains("--auth-mode insecure") {
        ...
        config.server_args = config
            .server_args
            .replace("--auth-mode insecure", "--auth-mode authenticated").to_string();
    }
    // -----------------------------------------------------------------
    ...
    if let Some(m) = &args.online_mode {
        ...
        config.online_mode = m.clone().to_string();
    ...
    if let Some(b) = &args.branch {
        config.branch = b.clone().to_string();
    }
    if let Some(v) = &args.game_version {
        config.game_version = v.clone().to_string();
    }
    if let Some(a) = &args.java_exec_args {
        config.java_exec_args = a.clone().to_string();
    }
    if let Some(a) = &args.server_args {
        config.server_args = a.clone().to_string();
    }
    if let Some(t) = &args.tunnel {
        config.tunnel_provider = Some(t.clone().to_string());
    }
