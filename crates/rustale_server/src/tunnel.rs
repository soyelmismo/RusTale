    ...
    if need_download {
        ...
        rustale_shared::download_file(
            ...
            |_phase, pct, speed, total, downloaded, eta, _step| {
                ...
                print!(
                    "\r[Tunnel] Downloading: {:.1}% ({}{}{})     ",
                    pct, speed, size_info, eta_info
                );
            ...
            line_res = lines.next_line() => {
                match line_res {
                    Ok(Some(line)) => {
                        ...
                        if let Some(mat) = re_url.find(&line_str) {
                            let url: String = mat.as_str().to_string();
                        ...
                        if line_str.contains("tunnel running")
                            || line_str.contains(".gl.joinmc.link")
                            || line_str.contains(".ply.gg")
                        {
                            if !notified {
                                let _ = on_ready.send("READY".to_string()).await;
                                notified = true;
                            }
                            if line_str.contains("address") || line_str.contains("tunnel running") {
                                println!("[Playit] {}", line_str);
                            }
                        }
                    }
                    Ok(None) => break,
